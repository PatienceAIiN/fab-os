...
|---|-----------|-----------|--------|
| 1 | Remove Canonical trademarks (name, logos, artwork) from the product identity | `packages/fabric-branding` diverts os-release, lsb-release, issue, legal, MOTD, dpkg origin; `image/Containerfile` never installs `ubuntu-wallpapers`, `plymouth-theme-ubuntu-*`, `ubuntu-artwork`, `ubuntu-pro-client` | done |
...
| 7 | Do not link GPL libraries into our Apache-licensed daemons in the same process | Fabric AI daemons are static Rust binaries talking to system components over IPC / exec | done |
