#!/usr/bin/env bash
# Rounded window corners, checked INSIDE the booted VM over SSH (vm profile: scripts/boot-vm.sh, hostfwd 2222->22). ADR-0019.
#   1. KWin lists the KDE-Rounded-Corners effect (kwin4_effect_shapecorners) in org.kde.kwin.Effects.loadedEffects.
...
BOOTED=0
echo "### Fab OS rounded corners VM test — $(date -u +%FT%TZ) — radius $R"
...
# ---------- 1. the effect is loaded
echo; echo "=== effect"
vm "dpkg-query -W -f 'package: \${Package} \${Version} \${Status}\n' fabos-rounded-corners 2>&1; grep -n '^kwin4_effect_shapecornersEnabled\|^\[Round-Corners\]\|^Size=\|^InactiveCornerRadius=' /etc/xdg/kwinrc"
...
shot() { # shot <remote png> <local png>: spectacle full screen, batch, no notification; waits for the file
  ...
  vm "test -s $1" || { echo "spectacle produced no $1 (session env: $(vm 'echo $WAYLAND_DISPLAY'))"; return 1; }
  $SCP "fabos@127.0.0.1:$1" "$2" >/dev/null; }
for scheme in $schemes; do
  ...
  $SCP "build/corners-kwin-$scheme.js" fabos@127.0.0.1:/tmp/corners-kwin.js >/dev/null
  ...
  echo "frame geometry ${GW}x${GH}+${GX}+${GY}; sampling corners vs ${R} px inside (build/corners-$scheme.png)"
  res=$(python3 - "build/corners-$scheme.png" "build/corners-$scheme-ref.png" "$GX" "$GY" "$GW" "$GH" "$R" <<'PY'
import sys
from PIL import Image
shot, ref, x, y, w, h, r = sys.argv[1], sys.argv[2], *map(int, sys.argv[3:8])
im = Image.open(shot).convert("RGB"); rf = Image.open(ref).convert("RGB")
def d(a, b): return sum((p - q) ** 2 for p, q in zip(a, b)) ** 0.5
...
top = im.getpixel((x + w // 2, y + 2)); ok = True
...
for k in ("TL", "TR", "BL", "BR"):
    ...
    print("%s corner%s=%s inside%s=%s outside%s=%s ref(no window)=%s  d(corner,inside)=%.0f d(corner,outside)=%.0f d(corner,ref)=%.0f  %s"
          % (k, corner[k], c, inside[k], i, outside[k], o, rp, d(c, i), d(c, o), d(c, rp), "ok" if good else "BAD"))
print("RESULT", "PASS" if ok else "FAIL")
PY
); echo "$res"
  echo "$res" | grep -q "^RESULT PASS" && verdict PASS "$scheme: all four corners transparent at radius $R, insides are the window" || verdict FAIL "$scheme: corner sampling (see the values above)"
