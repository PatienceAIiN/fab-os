...
#      builds, instantiates every ConvoDelegate kind / AiMark state and renders build/askbar-{bar,panel,feed}.png.
...
s = s[:-1] + '    Loader { source: Qt.resolvedUrl("../harness/Driver.qml"); onLoaded: { item.bar = root; item.convo = convo; item.dialog = panel; item.field = field; item.list = list } }\n}\n'
open(p, "w").write(s)
EOF
fail=0
run() {   # $1 label, $2 applet id, $3 timeout seconds, $4 extra XDG_DATA_DIRS prefix
  ...
  podman run --rm -e QT_QPA_PLATFORM=offscreen -e HOME=/tmp -e XDG_RUNTIME_DIR=/tmp/xdg -e XDG_DATA_DIRS="$4/usr/local/share:/usr/share" \
    -v "$T:/harness:ro,Z" -v "$OUT:/out:Z" -v "$PKG/contents/ui:/usr/share/plasma/plasmoids/in.patienceai.fabos.askbar/contents/ui:ro,Z" "$IMG" \
    bash -c "mkdir -p /tmp/xdg && chmod 700 /tmp/xdg; timeout $3 dbus-run-session plasmawindowed $2 2>&1; echo exit=\$?" > "$T/$2.log" 2>&1
  grep -vE "dbus-daemon|kglobalaccel|kf.windowsystem|propagateSizeHints|KWindowShadow|QProcess: Destroyed|Loading default layout|^qml: 396$|^$" "$T/$2.log"
}
...
echo "askbar-qml-test: $([ $fail = 0 ] && echo PASS || echo FAIL)  (renders: $OUT/askbar-{bar,panel,feed}.png)"
exit $fail
