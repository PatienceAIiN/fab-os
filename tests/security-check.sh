...
# Same style as tests/branding-check.sh: one PASS/FAIL line per check, exit 1 when anything failed.
...
chk "apparmor: fabos-voiced + fabos-llama enforce, fabos-agentd complain (documented path to enforce)" "R 'test -f /etc/apparmor.d/fabos-voiced && test -f /etc/apparmor.d/fabos-llama && test -f /etc/apparmor.d/fabos-agentd && ! grep -q complain /etc/apparmor.d/fabos-voiced && ! grep -q complain /etc/apparmor.d/fabos-llama && grep -q \"flags=(complain\" /etc/apparmor.d/fabos-agentd && grep -q \"Path to enforce\" /etc/apparmor.d/fabos-agentd'"
...
chk "root: no /etc/sudoers.d/fabos-agent (NOPASSWD rule removed)" "! R 'test -e /etc/sudoers.d/fabos-agent'"
chk "root: no NOPASSWD rule for rootexec anywhere in sudoers"  "! R 'grep -rE \"NOPASSWD.*rootexec\" /etc/sudoers /etc/sudoers.d 2>/dev/null' | grep -q ."
...
\"'"
chk "root: no polkit rule weakens in.patienceai.fabos.rootexec" "! R 'grep -rl in.patienceai.fabos.rootexec /etc/polkit-1/rules.d /usr/share/polkit-1/rules.d 2>/dev/null' | grep -q ."
...
chk "privileged files: no setuid file beyond the baseline"    "[ -z \"\$(R 'find / -xdev -perm -4000 -type f 2>/dev/null' | grep -vxF -f <(grep -v '^#' $BASE/suid-baseline.txt))\" ]"
chk "privileged files: no setgid file beyond the baseline"    "[ -z \"\$(R 'find / -xdev -perm -2000 -type f 2>/dev/null' | grep -vxF -f <(grep -v '^#' $BASE/sgid-baseline.txt))\" ]"
chk "privileged files: no file capability beyond the baseline" "[ -z \"\$(R 'getcap -r / 2>/dev/null' | grep -vxF -f <(grep -v '^#' $BASE/caps-baseline.txt))\" ]"
chk "privileged files: no Fab OS file is setuid/setgid/capability" "[ -z \"\$(R 'find /usr/lib/fabos /usr/share/fabos /usr/bin/fabos* -xdev \( -perm -4000 -o -perm -2000 \) -type f 2>/dev/null; getcap -r /usr/lib/fabos /usr/share/fabos 2>/dev/null')\" ]"
...
chk "permissions: no world-writable file/dir under /usr/lib/fabos /usr/share/fabos /etc/fabos" "[ -z \"\$(R 'find /usr/lib/fabos /usr/share/fabos /etc/fabos -xdev \( -type f -o -type d \) -perm -0002 2>/dev/null')\" ]"
...
chk "source: agent tests cover policy clamps, audit chain, rootexec, sandbox" "grep -q 'class SecurityUnits' $SRC/tests/agent-test.py && grep -q 'class PolicyDaemon' $SRC/tests/agent-test.py && grep -q 'test_audit_chain' $SRC/tests/agent-test.py && grep -q 'rootexec' $SRC/tests/agent-test.py"
