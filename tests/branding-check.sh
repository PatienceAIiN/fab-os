...
chk "quick settings probe prints one JSON line and has a --light mode (2 processes)" "R 'grep -q -- \"--light\" $QS/contents/code/status.sh && grep -q \"exec awk\" $QS/contents/code/status.sh && ! grep -q powerprofilesctl $QS/contents/code/status.sh && ! grep -q bluetoothctl $QS/contents/code/status.sh && grep -q lightCmd $QS/contents/ui/main.qml'"
