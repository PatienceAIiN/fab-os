...
chk "Fab OS session entry (no Plasma name)" "R 'grep -q ^Name=Fab\ OS /usr/local/share/wayland-sessions/fabos.desktop && grep -q SessionDir=/usr/local/share/wayland-sessions /etc/sddm.conf.d/10-fabos.conf'"
chk "Fab Wallet override present"          "R 'grep -q ^Name=Fab\ Wallet /usr/local/share/applications/org.kde.kwalletmanager5.desktop'"
...
chk "firefox from Mozilla repo"            "R 'apt-cache policy firefox' | grep -q packages.mozilla.org"
