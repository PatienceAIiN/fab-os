...
- **Small model, measured expectations.** On the agent's graded ladder (`tests/ladder`, the same checks the cloud providers pass 21/21), run inside the Fab OS image with the real model on 2026-09-15, the built-in model passed **6 of 6 easy and 4 of 5 medium tasks** (2 of 11 before the driver). It creates a folder and a file with the exact text, counts the files of a folder and answers in the asked form, opens Fab Terminal, writes today's date from the clock, copies a folder (source intact), opens Fab Editor and types where you can watch, sums a column across three CSV files, names the largest file under a tree, types a sentence into Fab Editor and saves it and fetches a local URL and saves the body unchanged. What still failed: l2-b (renames a folder of notes to another extension; the agent reported the failure itself). One run is a sample: the same task can pass one run and fail the next. The per-task table, and what failed in the checker's own words, are in [docs/LOW-RAM.md](docs/LOW-RAM.md). For hard tasks connect a cloud provider — it is one dropdown away, and everything else stays the same.
