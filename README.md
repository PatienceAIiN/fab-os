...
**Ollama is your own install, not Fab OS's.** Ollama is not in the Ubuntu archive and its installer downloads about 1 GB, so Fab OS does not bundle it. If you install it (`fabos ollama install` prints the official command and, with `--yes`, runs it as administrator after you type your password — never silently), pick **Ollama (on this computer)** in the dropdown: no key, `127.0.0.1:11434`, and the agent uses **the largest model you have pulled that fits your RAM** — up to 3B parameters on a 4 GB machine, 8B on 8 GB, 14B on 16 GB, 34B on 32 GB, larger above that (`fabos settings ollama.model llama3.1:8b` overrides). A model Ollama marks as tool-capable with 7B parameters or more runs the same free-form loop the cloud models run; smaller or tool-less models run the small-model driver, with tools described in text and the JSON answer parsed. `fabos ollama status` and `fabos ollama models` show what is there. On this release the built-in llama.cpp runs on the **CPU only**; Ollama is the path to your GPU.
