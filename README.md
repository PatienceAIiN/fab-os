...
A single **System-Wide AI** switch turns the whole thing off. Nothing leaves your machine except the requests you give the agent, sent only to the provider you configure yourself: **Anthropic (Claude), Google Gemini, OpenAI, DeepSeek, or the built-in local model** (llama.cpp — fully offline, no account). No telemetry, no accounts, no crash uploads.
