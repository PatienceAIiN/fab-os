...
| Top bar & dock | One **quick settings** group at the top-right corner — network glyph with the live download / upload rate (always shown while you are connected, `0 kB/s` when idle), Bluetooth, volume, battery **with its percentage**, and a bell — opens a wide dropdown card (36 gridUnits, about 650 px) that slides down from the bar without a flicker (the window opens at its final size; only the card moves, and the tiles fade in one after another). Three columns of large tiles — Wi-Fi with its network and signal, Bluetooth, a volume slider and a brightness slider with big round thumbs, a battery card with the percentage in large type and a Power saver / Balanced / Performance switch, Do Not Disturb, night light, screenshot, settings — and a footer with your interface, IP address and live speed. The tiles are **yours to arrange**: press the pencil to drag them into any order, make one small, medium or wide, remove it or add it back (notifications, power profile and network speed wait as extras). The bell opens a notification history of the same width with a Clear all button. The stock network / Bluetooth / volume / battery tray icons are gone (not merely hidden). The **clock** is Fab OS's own — day, date and time on one bold line in the same size as everything else in the bar (one **Bar size** setting: Small / Medium / Large). The **dock** draws its start button, the pinned apps and "peek at the desktop" at one size with an even 12 px between every item and magnifies them under the pointer like macOS (hovered 1.6x, neighbours 1.3x / 1.1x; one switch shared with the bar; the dock's width never changes while you hover) with a bounce on launch. Running apps are marked like Windows 11 and macOS: a small accent dot under each running app (two for a group, dimmer when minimised), a short accent bar and a soft highlight under the active window, nothing under a plain launcher |
...
**Where to install.** *Erase disk* is preselected. **Encryption is your choice** on the same page: tick *Encrypt system* and choose a passphrase in the two boxes (on the 1.0 image the box starts ticked; from the next image it starts unticked). With encryption on, the installer creates an EFI system partition, a small unencrypted `/boot` (2 GB, holding the kernel and start-up files — the same layout Ubuntu's own installer uses) and one encrypted LUKS2 volume for everything else. **You will be asked for this passphrase at every boot**, before the login screen; there is no way to recover a forgotten one, so pick something you will remember and, if you like, write it down somewhere safe. Leave *Encrypt system* unticked if you would rather have no passphrase (same layout with `/` unencrypted; the system then starts straight to the login screen). *Replace a partition*, *Install alongside* and *Manual partitioning* remain available. The firmware boot entry is registered as `ubuntu` (Ubuntu's Secure-Boot-signed GRUB requires that directory name — [docs/decisions/ADR-0021](docs/decisions/ADR-0021-installer-boot-layout.md)); the boot menu entry itself says *Fab OS*.
