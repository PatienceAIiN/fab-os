...
def build_icon_theme(out, conf, marks):
    ...
    idx = ["[Icon Theme]", "Name=FabOS",
           "Comment=%s icons by %s (%s): Google Material Symbols on Fab OS tiles for apps, monochrome for status; everything else from Breeze" % (conf["DISTRO_NAME"], conf["VENDOR_NAME"], conf["HOME_URL"]),
