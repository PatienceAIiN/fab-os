...
// "Ask me to do…" bar that lives on the Fab OS desktop. Hands the request to the Fab OS agent (fabric-agentd)
// through the `fabric` CLI and shows the agent's live status underneath.
PlasmoidItem {
    id: root
    ...
    P5Support.DataSource {
        ...
        onNewData: (source, data) => {
            ...
            if (source.indexOf("fabric do ") === 0) {
                root.lastTask = out.length ? out : "Task started"
                root.status = root.lastTask
            } else if (source.indexOf("fabric status") === 0 && out.length) {
                root.status = out
            }
        }
    }
    Timer { interval: 4000; running: true; repeat: true; triggeredOnStart: true
            onTriggered: exec.connectSource("fabric status --brief 2>/dev/null || echo 'Fab OS agent is starting…'") }
    ...
    Rectangle {
        ...
        RowLayout {
            ...
            spacing: 10
            Kirigami.Icon { source: "fabric-os"; Layout.preferredWidth: 30; Layout.preferredHeight: 30 }
            QQC2.TextField {
                id: field
                ...
                onAccepted: {
                    ...
                    exec.connectSource("fabric do " + root.shellQuote(t) + " 2>&1 | head -1")
                    text = ""
                }
            }
            QQC2.Button {
            ...
            QQC2.ToolButton {
                ...
                onClicked: exec.connectSource("setsid -f fabric-command-center >/dev/null 2>&1; echo opened")
            }
        }
        Text {
            ...
            MouseArea { anchors.fill: parent; cursorShape: Qt.PointingHandCursor; onClicked: exec.connectSource("setsid -f fabric-command-center >/dev/null 2>&1; echo opened") }
        }
    }
}
