...
Talks to fabric-agentd over its local HTTP API. PyQt6. Launch: fabric-command-center [--ask]
"""
...
class CommandCenter(QMainWindow):
    def __init__(self, focus_ask=False):
        super().__init__()
        self.setWindowTitle(APP + " Command Center"); self.setWindowIcon(QIcon.fromTheme("fabric-os")); self.resize(1180, 740); self.current = None
    ...
    def report_problem(self):
        ...
        subprocess.Popen(["fabric-feedback", "--type", "bug", "--prefill", pre])
    ...
    def refresh(self, select_id=None):
        try:
            ...
            self.statusBar().showMessage("fabric-agentd not running: %s  (systemctl --user start fabric-agent)" % e); return
...
def main():
    app = QApplication(sys.argv); app.setApplicationName(APP + " Command Center"); app.setDesktopFileName("fabric-command-center"); app.setStyleSheet(STYLE)
    f = QFont("Inter", 10); app.setFont(f)
    w = CommandCenter(focus_ask="--ask" in sys.argv); w.show(); sys.exit(app.exec())
