#!/usr/bin/env python3
"""fabric-feedback — send feedback or a bug report about Fab OS to Patience AI (replaces KDE's user-feedback and
bug-report flows, which pointed at KDE). GUI (PyQt6) by default; --cli for terminals and scripts.

  fabric-feedback                         open the dialog
  fabric-feedback --type bug --prefill "text"
  fabric-feedback --cli --type feedback --subject "..." --message "..." [--email you@x]
"""
import argparse, json, os, platform, socket, subprocess, sys

SOCK = os.environ.get("FABRIC_FEEDBACK_SOCK", "/run/fabric/feedback.sock")
...
def system_info(include_logs=False):
    ...
    for key, cmd in (("plasma", ["plasmashell", "--version"]), ("fabric_release", ["cat", "/etc/fabric/release"]), ("memory", ["free", "-h"])):
        try:
            info[key] = subprocess.run(cmd, capture_output=True, text=True, timeout=5).stdout.strip().replace("\n", " | ")[:300]
        except Exception:
            pass
    try:
        info["agent"] = subprocess.run(["fabric", "status", "--brief"], capture_output=True, text=True, timeout=5).stdout.strip()[:200]
    except Exception:
        pass
    if include_logs:
        try:
...
def gui(args):
    ...
    def do_send():
        ...
        if r.get("ok"):
            ...
            QMessageBox.critical(d, "Fab OS", "Could not send the report: %s\n\nThe feedback channel needs /etc/fabric/feedback.env to be configured." % r.get("error"))
