#!/usr/bin/env python3
"""fabric-feedback-relay — system service that delivers Fab OS feedback / bug reports to Patience AI.

Runs as root via systemd socket activation on /run/fabric/feedback.sock so the mail credentials in
/etc/fabric/feedback.env (root-only) are never readable by user processes. One JSON object per connection:
...
ENV_PATH = os.environ.get("FABRIC_FEEDBACK_ENV", "/etc/fabric/feedback.env")
SOCK_PATH = os.environ.get("FABRIC_FEEDBACK_SOCK", "/run/fabric/feedback.sock")
...
def main():
    if os.environ.get("LISTEN_FDS"):
        ...
        srv.listen(8)
    log("fabric-feedback-relay ready (env=%s)" % ENV_PATH)
