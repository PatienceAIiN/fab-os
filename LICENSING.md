...
- **Trademarks**: Ubuntu (Canonical), KDE/Plasma (KDE e.V.), Firefox (Mozilla Foundation) are named only for identification;
...
- **Firefox**: Mozilla's own unmodified package from packages.mozilla.org; Fab OS does not rebuild, rename or re-brand it
  (Mozilla's documented enterprise-policy file only sets unlocked first-run defaults, ADR-0018). Firefox is a trademark of the
  Mozilla Foundation; Fab OS ships Mozilla's own unmodified build and is not endorsed by Mozilla.
