...
- Start-up: the maker's logo stays on screen while Fab OS starts, with a spinner and the Fab OS wordmark; on an encrypted disk the password box is now a real box with bullets, a label and a Caps Lock warning; shutting down shows "Shutting down safely…".
...
Known limits: the new dock, login screen and leave screen appear after the next log-in. The disk-password box cannot show what you type (the login screen has the eye, the start-up screen cannot). Turning the start-up password off keeps your files encrypted at rest but lets anyone who starts the computer use it — the setting says so before it acts.
...
- Security baseline for organisations: actions as administrator ask for your password, shell steps run in a sandbox, administrators can clamp the product with one policy file, the activity log is tamper-evident, and every component is listed for audit.
...
## 1.0-5 — 15 September 2026 (built, not published)

A disc image was built for this revision and held back when the installer of the previous image failed on a real
device. Its changes — Firefox returning in place of Brave, rounded corners on every window, a stronger step-by-step
mode for the built-in offline model, and a second pass on the top bar and dock — shipped in 1.0-6.
...
- The system is hardened (kernel settings, application confinement), and a list of every component is generated for audit.
...
- Voice: the microphone path works on real hardware, the assistant never repeats a sentence, the offline voice is audible, and a voice check-up tells you what is wrong when something is.
...
The first public Fab OS 1.0 disc image, published the same day as the first build, with these changes on top of it:
...
- A calm KDE Plasma desktop on Ubuntu 26.04 with the Fab OS look: its own window frame, monochrome status icons, the Inter typeface and an 11 pt interface.
...
- Fab AI Controls, Fab Updates, Fab Feedback, Fab Terminal, Fab Files, Fab Editor, Fab Software, Fab Photos and Fab Documents, with Firefox, LibreOffice, VLC and a weather app.
