...
# Update 1.0-8 (round 8, over the air) — 2026-09-17

No ISO was rebuilt. Everything below ships as package revision **1.0-8** through the signed apt channel. The owner's second
round of real-laptop reports: high battery use and lag when opening apps, the ask-bar microphone doing nothing, new chats
stuck as "queued", a KDE/Plasma-branded screen after reboot and remaining KDE product names, the disk-password switch
seemingly not taking effect, plus requests for Research/Computer-use switches, a scheduler, fingerprint and camera support,
optional encryption in the installer and a readable installer sidebar, a public changelog and a GitHub release per update.

## Tracks, root causes, reviews (each implemented by one agent and reviewed by a separate skeptical agent that re-ran the evidence)

| track | what was found | what shipped | review |
|---|---|---|---|
| performance and battery (fabos-tuning, fabos-voice, fabos-desktop) | idle desktop measured in a 2 GB VM: the always-on "Hey Fab" capture chain was the largest steady consumer (about 1 % of a core, ~150 wake-ups/s); a translucency key KWin never reads; app launches not delayed by the session itself | Performance mode with five choices (Power saver, Balanced, Performance, Gaming, Server) driving power-profiles-daemon, KWin tearing/VRR/effects, PowerDevil and the listener, with the user's own settings snapshotted and restored; speech-gated listener with an on / battery-off / off policy; I/O scheduler rules; faster user-service stop; a measured `tests/perf-vm.sh` with budgets | **fixed then approved**: Server → Gaming carried the never-sleep/lid settings along (restore now runs first); VRR policy is restored per output; the speech gate lost quiet speech in a synthetic test and was re-tuned to lose none |
| mic and concurrent chats (fabos-agent, fabos-voice) | a running task held an execution slot for its whole life, including the hours it may wait for approval, so two waiting chats blocked every new chat; the mic showed "Listening…" before the recorder opened, hid failures, and refused below 600 MB free while whisper's real peak is 174 MB | slots held only while a step executes; per-chat serialisation; queued reasons shown; mic path with a live level meter, a progress file and named failures; memory gate 208 MB; "Allow Fab OS to use the microphone" switch (off for fresh installs, migrated on for installs already using voice) gating recording, the wake-word listener and the speech endpoints | **fixed then approved**: a task re-queued during a daemon restart could have re-run from scratch; Stop during a slot wait still executed the step; the listener took up to 30 minutes to notice the permission switch |
| Research / Computer-use switches (fabos-agent) | none before; requested | animated pill switches per chat in the ask bar and Fab AI Controls from one shared design file; off removes the tools from the model's list and refuses stray calls | **fixed then approved**: with a switch off the model could still reach GUI actions or the network through the shell; the shell is now gated by the same rule (a word-based classifier, limits documented) |
| scheduler (fabos-agent) | none before; requested | offline natural-language parser (English and Indian-English; unclear times are confirmed first), reminders with Done / Snooze / Open, a Schedule page, a "Today" notification at login, reminders by mail to yourself | **fixed then approved**: the mail intake would have re-read its own confirmation reply every ten minutes (duplicate reminders, mail loop); one popup per pass |
| branding and hardware (fabos-branding, fabos-hardware, fabos-desktop) | 50 visible KDE/Plasma/K-named strings on the shipped desktop; users whose earlier settings pinned the Breeze theme got its "Plasma" splash; the 1.0-6 login theme's fallback was the blue KDE screen | overrides generator, one-time migration of pinned defaults, diversions of two KDE-named plugins; `fabos-hardware` (fingerprint stack with password fallback, camera stack, Fab Camera); mic volume and mic/camera in-use indicators | **fixed then approved**: the binary rebrand hook would have written a stale original over any Plasma binary Ubuntu later upgraded (also on 1.0-7 machines) — it now verifies against the package's checksums; fingerprint re-enabled after a user opt-out; PAM hints on the login screen shown as hints, not errors |
| disk-password diagnostics (fabos-agent) | the switch showed only the stored intent, not what start-up would do | `diagnose` and `repair`, real state in the Start-up row with "Fix now", a terminal command, a how-to page | **fixed then approved**: repair could remove a live key slot before the rebuild was proven (a working key is now kept); an unknown verdict is refused |
| installer (ISO tree; reaches users with the next image) | Calamares 3.3 ignores the 3.2 spelling of the sidebar colour keys we shipped, so it painted black on black; encryption was pre-selected | keys renamed; encryption opt-in; state-aware install driver | **approved** (verified against the Calamares 3.3.14 source and the image's library) |
| changelog and releases | none before; requested | user-facing changelog file rendered to the site with a footer link on every page and pagination; GitHub release script gains an over-the-air mode | **fixed then approved**: four public claims corrected (including that 1.0-5 did reach installed systems) |
