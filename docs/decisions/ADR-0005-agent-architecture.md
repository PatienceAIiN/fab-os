...
- Providers behind one interface: Claude (Anthropic SDK, default model claude-opus-5, server-side refusal
  fallbacks) and any endpoint speaking the common chat-completions HTTP protocol (the other cloud providers in the
  PROVIDERS table, Gemini, local llama-server). Keys via systemd-creds.
