...
- `brave-browser 1.95.101` deb: `usr/share/applications/brave-browser.desktop` (`Name=Brave Web Browser`,
  ...
  `x-www-browser`/`gnome-www-browser` alternatives and installs an AppArmor profile (`userns` allowed) unless one exists —
  Ubuntu 26.04 already ships `/etc/apparmor.d/brave` for `/opt/brave.com/brave/brave`, so the Chromium sandbox works under
  `kernel.apparmor_restrict_unprivileged_userns = 1`.
- Packaging quirk (security pass): the deb's postinst and daily cron script are Chromium's installer template with
  **Google's** repository constants left in (`REPOCONFIG=… dl.google.com/linux/chrome/deb/`, `gen_sources_content` →
  `dl.google.com/linux/chrome-stable/deb/`, `install_key` → Google's key as `/usr/share/keyrings/brave-browser.gpg`). The
  postinst `exit 0`s before that block ("Don't add the Chrome repo (brave/brave-browser#54299)"), so installing Brave adds
  no Google source. The cron script (`/etc/cron.daily/brave-browser` → `/opt/brave.com/brave/cron/brave-browser`) would run
  `install_key` unconditionally and add the Chrome source when `/etc/default/brave-browser` says `repo_add_once="true"`.
  Fab OS neutralises this twice over: it ships `/etc/default/brave-browser` with `repo_add_once="false"` and
  `repo_reenable_on_distupgrade="false"`, and the image has no `cron`/`anacron`, so `cron.daily` never runs.
