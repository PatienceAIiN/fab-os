...
1. **Presets, not fields.** `mail.provider` ∈ {gmail, outlook, yahoo, zoho, icloud, other} selects a preset in
   ...
   agent (the feedback package keeps its own Brevo configuration untouched).
   ...
   | Outlook / Hotmail | smtp-mail.outlook.com **587 STARTTLS** | outlook.office365.com **993 SSL/TLS** | "Modern Auth / OAuth2 required"; an app password where the account allows one | support.microsoft.com, "POP, IMAP, and SMTP settings for Outlook.com" |
   | Yahoo Mail | smtp.mail.yahoo.com **465 SSL** (587 also documented) | imap.mail.yahoo.com **993 SSL** | generated app password | help.yahoo.com, "IMAP server settings for Yahoo Mail" (SLN4075) |
   ...
   The hosts and ports above were read from those pages; the *logins themselves* were not exercised against live
   accounts in this change (no credentials in the build), so `POST /mail/test` is the user's proof.
...
4. **`POST /mail/test`** `{provider, address, password?, mail.smtp_host?, …}` → `{ok, detail, smtp:{ok,detail},
   ...
   stored credential otherwise. It distinguishes **wrong password / app password required** (SMTP 535/534,
   `SMTPAuthenticationError`; IMAP `AUTHENTICATIONFAILED` / "Invalid credentials") — phrased as "wrong password — Gmail
   needs an app password, not your account password" for providers in the table, "the server refused the login" for
   others — from **cannot reach host:port (reason)**. With no IMAP server (provider *other*) IMAP is reported as skipped
   and SMTP decides. The password is never logged: the activity row records provider, address and the verdict only.

5. **Fab AI Controls → Settings → Mail** is: provider dropdown (Gmail first), address, one **Sign in** button. With
   Google OAuth available it runs the browser flow and shows the animated check; otherwise it reveals the app-password
   field with the provider's 3-step hint ("Google Account → Security", "2-Step Verification", "App passwords …") and a
   **Check connection** button with the same animated check / shake-on-failure pattern as the provider check. Save is
   disabled until the check passed for exactly the values in the form (or "Require a successful check" is unticked
   under Advanced). The welcome wizard gained a Mail page whose button opens `fabos-command-center --settings mail`.
...
- Without the distributor's Google client, Gmail works through an app password (2-Step Verification required by
  Google); the UI says so instead of failing silently. Outlook.com personal accounts may refuse basic auth
  altogether (Microsoft: "Modern Auth / OAuth2 required"); the check reports that as a wrong-password class failure
  and the ADR keeps an OAuth path for Microsoft as future work.
