...
- The baseline, re-measured with the new harness (`tests/local-driver-image.sh --label before`, HEAD daemon + wrapper):
  **L1 2/6, L2 0/5**, the same two passes as in the QA record (count the files, open the terminal). Every other task
  ended after **one** tool call with a done-message: `list_dir` then "I will now create the file" (l1-a), `open_app kate`
  then "I have opened Fab Editor for you. Type the word hello" (l1-f), `read_file` of one CSV then "Now I will add the
  amount column" (l2-a). Tasks took 6–78 s each.
...
- The cloud path is unchanged: same loop, same request shape (`temperature 0.2`, no `top_p`, no `tool_choice`), covered
  ...
  repair, one tool per turn, deterministic and self-check retries, the honest give-up, the outcome repair step, the
  driver selection and the network probe without a model.
