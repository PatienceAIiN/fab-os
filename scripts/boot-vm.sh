#!/usr/bin/env bash
# Boot the Fabric OS disk in QEMU/KVM with UEFI (OVMF), under the resource guard so the host never hangs.
# Usage: scripts/boot-vm.sh [--headless] [--autotest] [--mem MB] [--cpus N] [--timeout S] [--fresh-vars]
set -euo pipefail
HERE=$(cd "$(dirname "$0")/.." && pwd); cd "$HERE"
MEM=2048; CPUS=4; HEADLESS=0; AUTOTEST=0; TIMEOUT=0; DISK=build/fabric-vm.img
while [ $# -gt 0 ]; do case $1 in
  --headless) HEADLESS=1;; --autotest) AUTOTEST=1;; --mem) MEM=$2; shift;; --cpus) CPUS=$2; shift;;
  --timeout) TIMEOUT=$2; shift;; --fresh-vars) rm -f build/OVMF_VARS.fd;; --disk) DISK=$2; shift;; *) echo "unknown arg $1"; exit 2;; esac; shift; done
...
args=( -name "Fabric OS" -machine q35,accel=$ACCEL -cpu $CPU -smp "$CPUS" -m "$MEM"
...
if [ $AUTOTEST = 1 ]; then args+=( -no-reboot -smbios type=11,value=io.systemd.boot.kernel-cmdline-extra=fabric.autopoweroff\ console=ttyS0 ); fi
