...
MEM=2048; CPUS=4; HEADLESS=0; AUTOTEST=0; TIMEOUT=0; GL=0; DISK=build/fabos-vm.img
while [ $# -gt 0 ]; do case $1 in
  --headless) HEADLESS=1;; --autotest) AUTOTEST=1;; --mem) MEM=$2; shift;; --cpus) CPUS=$2; shift;;
  --timeout) TIMEOUT=$2; shift;; --gl) GL=1;; --fresh-vars) rm -f build/OVMF_VARS.fd;; --disk) DISK=$2; shift;; *) echo "unknown arg $1"; exit 2;; esac; shift; done
...
if [ $HEADLESS = 1 ]; then args+=( -display none -device virtio-vga ); elif [ $GL = 1 ]; then args+=( -device virtio-vga-gl -display gtk,gl=on,show-cursor=on ); else args+=( -device virtio-vga -display gtk,show-cursor=on,zoom-to-fit=on ); fi
